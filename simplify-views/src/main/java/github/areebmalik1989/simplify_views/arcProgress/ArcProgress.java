package github.areebmalik1989.simplify_views.arcProgress;
...
import github.areebmalik1989.simplify_views.R;
...
public class ArcProgress extends View {
    ...
    // Changed % as default suffix by Areeb
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // Updated default text size from 40 to 75 by Areeb
        default_text_size = Utils.sp2px(getResources(), 75);
        default_suffix_text_size = Utils.sp2px(getResources(), 15);
        default_suffix_padding = Utils.dp2px(getResources(), 4);
        // Removed % as default suffix by Areeb
        default_suffix_text = "";
    ...
    public void setProgress(float progress) {
        ...
        // Changed pattern from #.## to #.# by Areeb
        this.progress = Float.valueOf(new DecimalFormat("#.#").format(progress));

        if (this.progress > getMax()) {
            // Commented by Areeb to fix wrong output
    ...
    public void setTextSize(float textSize) {
        this.textSize = Utils.sp2px(getResources(), textSize);
        this.invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // added if-else by Areeb to stop making complete circle
        float finishedSweepAngle;
        if(!(progress > getMax())){
            finishedSweepAngle = progress / (float) getMax() * arcAngle;
        } else {
            finishedSweepAngle= getMax() / (float) getMax() * arcAngle;
        }
